...
예를 들어 Rect와 Circle 이라는 2개의 타입이 있을 때, Shape 인터페이스를 구현하기 위해서는 아래와 같이 각 타입별로 2개의 메서드를 구현해 주면 된다.
*/
...
따라서 Rect와 Circle 처럼 Shape 인터페이스를 구현한 타입 객체들을 파라미터로 받을 수 있다.
...
func main() {
	...
	Empty interface는 메서드를 전혀 갖지 않는 빈 인터페이스로서, Go의 모든 Type은 적어도 0개의 메서드를 구현하므로,
	흔히 Go에서 모든 Type을 나타내기 위해 빈 인터페이스를 사용한다.
	...
	*/
	...
	x는 T 타입에 속한다는 점을 확인(assert)하는 것으로 이러한 표현을 "Type Assertion"이라 부른다.
	...
	*/
	...
	j := a.(int) // j는 int 타입, 값은 1 >> a는 nil이 아니고 int형임 아니면 런타임에러 발생
	println(i)   // 포인터주소 출력
	println(j)   // 1 출력
}

func printX(v interface{}) {
	fmt.Println(v) //Tom
}
