...
/**
1. Go 채널
Go 채널은 그 채널을 통하여 데이타를 주고 받는 통로라 볼 수 있는데, 채널은 make() 함수를 통해 미리 생성되어야 하며, 채널 연산자 <- 을 통해 데이타를 보내고 받는다.
채널은 흔히 goroutine들 사이 데이타를 주고 받는데 사용되는데, 상대편이 준비될 때까지 채널에서 대기함으로써 별도의 lock을 걸지 않고 데이타를 동기화하는데 사용된다.

아래 예제는 정수형 채널을 생성하고, 한 goroutine 에서 그 채널에 123이란 정수 데이타를 보낸 후, 이를 다시 메인 루틴에서 채널로부터 123 데이타를 받는 코드이다.
채널을 생성할 때는 make() 함수에 어떤 타입의 데이타를 채널에서 주고 받을지를 미리 지정해 주어야 한다.
채널로 데이타를 보낼 때는 채널명 <- 데이타 와 같이 사용하고, 채널로부터 데이타틀 받을 경우는 <- 채널명 와 같이 사용한다.
아래 예제에서 메인 루틴은 마지막에서 채널로부터 데이타를 받고 있는데, 상대편 goroutine에서 데이타를 전송할 때까지는 계속 대기하게 된다.
...
func main() {
	...
	//익명함수를 사용한 고루틴
	go func() { //
		time.Sleep(time.Second * 4) // business logic...
		ch <- 123                   //채널에 123을보낸다.
	}()

	println("채널로부터 123 받기 전 ")
	var i int
	i = <-ch //채널로부터 123을 받는다. (채널에서 주는 리턴값이 올때까지 대기)
	println(i)
	println("채널에서 데이터 받은후 ")

	/**
	Go 채널은 수신자와 송신자가 서로를 기다리는 속성때문에, 이를 이용하여 (다음 예제와 같이) Go루틴이 끝날 때까지 기다리는 기능을 구현할 수 있다.
	즉, 익명함수를 사용한 한 Go 루틴에서 어떤 작업이 실행되고 있을 때, 메인루틴은 <-done 에서 계속 수신하며 대기하고 있게 된다.
	익명함수 Go 루틴에서 작업이 끝난 후, done채널에 true를 보내면, 수신자 메인루틴은 이를 받고 프로그램을 끝내게 된다.
	*/
	done := make(chan bool)
	go func() {
		for i := 0; i < 10; i++ {
			fmt.Println(i) //business logic...
		}
		done <- true //채널에 끝났다고 알림
	}()
	...
	위의 예제에서의 Go 채널은 Unbuffered Channel로서 이 채널에서는 하나의 수신자가 데이타를 받을 때까지 송신자가 데이타를 보내는 채널에 묶여 있게 된다.
	...
	왜냐하면 메인루틴에서 채널에 1을 보내면서 상대편 수신자를 기다리고 있는데, 이 채널을 받는 수신자 Go루틴이 없기 때문이다.
	*/
	...
	/**
	3. 채널 파라미터
	채널을 함수의 파라미터로 전달할 때, 일반적으로 송수신을 모두 하는 채널을 전달하지만,
	...
	*/
	...
	채널 수신에 사용되는 <- ch 은 두개의 리턴값을 갖는데, 첫째는 채널 메시지이고, 두번째는 수신이 제대로 되었는가를 나타낸다.
	...
	*/
	...
	방법1은 무한 for 루프 안에서 if 문으로 수신 채널의 두번째 파라미터를 체크하는 방식이고,
	방법2는 방법1과 동일한 표현이지만, for range문으로 보다 간결하게 표현한 것이다.
...
func sendChan(ch chan<- string) {
	ch <- "Data"
	// x := <-ch // 에러발생, 송신전용 채널에 수신을 시도하므로 에러가 발생한다.
}
